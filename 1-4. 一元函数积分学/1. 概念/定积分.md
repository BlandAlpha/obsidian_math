...
尝试将 $g(i, n)$ 转化为 $f( \frac{i}{n} ) \frac{1}{n}$ 的形式。如果可以，则直接使用定积分。 

若分子**或**分母是关于 $n,i$ 的函数，且均为**齐次式**，则可以凑成 $f( \frac{i}{n} ) \frac{1}{n}$ 。如：

$$
\frac{n+i}{n^{2}+i^{2}} = \frac{n^{2}+in}{n^{2}+i^{2}} \cdot \frac{1}{n} = \frac{1+\frac{i}{n}}{1+(\frac{i}{n})^2}
$$

又如：

$$
\frac{1}{\sqrt{n^{2}+ni}} = \frac{1}{\sqrt{1+ \frac{i}{n}}} \cdot
$$